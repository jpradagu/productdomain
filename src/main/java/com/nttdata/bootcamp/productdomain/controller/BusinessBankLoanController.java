...
import com.nttdata.bootcamp.productdomain.service.BusinessBankLoanService;
...
@RestController
@RequestMapping("/api/product/business-bank-loan")
public class BusinessBankLoanController {
    @Autowired
    private BusinessBankLoanService loanService;

    @GetMapping
    public Mono<ResponseEntity<Flux<BusinessBankLoan>>> findAll() {
        return Mono.just(ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(loanService.findAll()));
    }

    @GetMapping("/{id}")
    public Mono<ResponseEntity<BusinessBankLoan>> findById(@PathVariable String id) {
    ...
    @PostMapping
    public Mono<ResponseEntity<Map<String, Object>>> create(@Valid @RequestBody Mono<BusinessBankLoan> monoAccount) {
    ...
    @PutMapping("/{id}")
    public Mono<ResponseEntity<BusinessBankLoan>> update(@RequestBody BusinessBankLoan account, @PathVariable String id){
