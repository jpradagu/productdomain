...
import com.nttdata.bootcamp.productdomain.service.BusinessBankAccountService;
...
@RestController
@RequestMapping("/api/product/business-bank-account")
public class BusinessBankAccountController {

    @Autowired
    private BusinessBankAccountService accountService;

    @GetMapping
    public Mono<ResponseEntity<Flux<BusinessBankAccount>>> findAll() {
        return Mono.just(ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(accountService.findAll()));
    }

    @GetMapping("/{id}")
    public Mono<ResponseEntity<BusinessBankAccount>> findById(@PathVariable String id) {
    ...
    @PostMapping
    public Mono<ResponseEntity<Map<String, Object>>> create(@Valid @RequestBody Mono<BusinessBankAccount> monoAccount) {
    ...
    @PutMapping("/{id}")
    public Mono<ResponseEntity<BusinessBankAccount>> update(@RequestBody BusinessBankAccount account, @PathVariable String id) {
    ...
    public Mono<ResponseEntity<Flux<BusinessBankAccount>>> findAllByCustomer(String id) {
